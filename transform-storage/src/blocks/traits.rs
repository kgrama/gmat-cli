...
use crate::block::{encode_e0m4, decode_e0m4, encode_e1m7, decode_e1m7};

/// Trait for element count dependent types (u8 for 8 elements, u16 for 16 elements)
pub trait ElementMask: Copy + Default + From<u8> + Into<u64> 
...
impl ElementMask for u8 {
    ...
    fn write_le<W: Write>(&self, w: &mut W) -> Result<()> { w.write_all(&[*self]) }
    fn read_le<R: Read>(r: &mut R) -> Result<Self> { 
...
impl ElementMask for u16 {
    ...
    fn write_le<W: Write>(&self, w: &mut W) -> Result<()> { w.write_all(&self.to_le_bytes()) }
    fn read_le<R: Read>(r: &mut R) -> Result<Self> { 
...
impl EncodingStrategy for E0M4 {
    ...
    fn encode(offset: f32) -> u8 { encode_e0m4(offset) }
    fn decode(raw: u8) -> f32 { decode_e0m4(raw) }
}
...
impl EncodingStrategy for E1M7 {
    ...
    fn encode(offset: f32) -> u8 { encode_e1m7(offset) }
    fn decode(raw: u8) -> f32 { decode_e1m7(raw) }
}
