...
use super::types::*;
use super::utils::{compute_superblock_scales, select_quant_with_fallback, quantize_superblocks, quantize_legacy_blocks};
use super::kquant::{compute_scales_standard, compute_q6k_scales, compute_q2k_scales, compute_q3k_scales, encode_q4k_block, encode_q5k_block, encode_q6k_block, encode_q2k_block, encode_q3k_block, Q4_K_CONFIG, Q5_K_CONFIG, Q6_K_CONFIG, Q2_K_CONFIG, Q3_K_CONFIG};
use super::iquant::{compute_iq4xs_scales, compute_iq4nl_scale, encode_iq4xs_block, encode_iq4nl_block, IQ4_XS_CONFIG, IQ4_NL_CONFIG};
...
pub fn quantize_to_gguf(
    ...
    let data = match actual_type {
        ...
        GgufQuantType::Q8_0 => quantize_legacy_blocks(&all_blocks, rows, cols, gmat_block_size, 34, legacy::blocks_to_q8_0),
        GgufQuantType::Q4_0 => quantize_legacy_blocks(&all_blocks, rows, cols, gmat_block_size, 18, legacy::blocks_to_q4_0),
        GgufQuantType::Q4_1 => quantize_legacy_blocks(&all_blocks, rows, cols, gmat_block_size, 20, legacy::blocks_to_q4_1),
        GgufQuantType::Q5_0 => quantize_legacy_blocks(&all_blocks, rows, cols, gmat_block_size, 22, legacy::blocks_to_q5_0),
        GgufQuantType::Q5_1 => quantize_legacy_blocks(&all_blocks, rows, cols, gmat_block_size, 24, legacy::blocks_to_q5_1),

        // K-quants with scale optimization
        GgufQuantType::Q4_K_S | GgufQuantType::Q4_K_M => {
            quantize_kquant_with_opt(&all_blocks, rows, cols, gmat_block_size, scale_opt, Q4_K_CONFIG.block_bytes, encode_q4k_superblock)
        }
        GgufQuantType::Q5_K_S | GgufQuantType::Q5_K_M => {
            quantize_kquant_with_opt(&all_blocks, rows, cols, gmat_block_size, scale_opt, Q5_K_CONFIG.block_bytes, encode_q5k_superblock)
        }
        GgufQuantType::Q6_K => {
            quantize_superblocks(&all_blocks, rows, cols, gmat_block_size, Q6_K_CONFIG.block_bytes, |out, ctx| {
        ...
        GgufQuantType::Q2_K => {
            quantize_superblocks(&all_blocks, rows, cols, gmat_block_size, Q2_K_CONFIG.block_bytes, |out, ctx| {
        ...
        GgufQuantType::Q3_K_S | GgufQuantType::Q3_K_M | GgufQuantType::Q3_K_L => {
            quantize_superblocks(&all_blocks, rows, cols, gmat_block_size, Q3_K_CONFIG.block_bytes, |out, ctx| {
                let (d, _) = compute_superblock_scales(ctx.block_refs);
        ...
        GgufQuantType::IQ4_XS => {
            quantize_superblocks(&all_blocks, rows, cols, gmat_block_size, IQ4_XS_CONFIG.block_bytes, |out, ctx| {
                let (d, _) = compute_superblock_scales(ctx.block_refs);
        ...
        GgufQuantType::IQ4_NL => {
            quantize_superblocks(&all_blocks, rows, cols, gmat_block_size, IQ4_NL_CONFIG.block_bytes, |out, ctx| {
    ...
    F: Fn(&mut [u8], f16, f16, &[u8; 8], &[u8; 8], &[&AnyBlock], usize) + Sync,
{
    quantize_superblocks(all_blocks, rows, cols, gmat_block_size, bytes_per_sb, |out, ctx| {
        ...
        let scales = match scale_opt {
            ScaleOptimization::Standard => compute_scales_standard(ctx.block_refs, d, dmin, ctx.gmat_block_size),
            ScaleOptimization::Trellis { lambda } => {
                optimize_group_scales_trellis(ctx.block_refs, d, dmin, None, lambda, ctx.gmat_block_size)
            }
            ScaleOptimization::TrellisDual { .. } => unimplemented!("TrellisDual requires paired rows"),
        };
        let mins = scales;
        encode_fn(out, d, dmin, &scales, &mins, ctx.block_refs, ctx.gmat_block_size);
    })
}

#[inline]
fn encode_q4k_superblock(out: &mut [u8], d: f16, dmin: f16, scales: &[u8; 8], mins: &[u8; 8], blocks: &[&AnyBlock], bs: usize) {
    encode_q4k_block(out, d, dmin, scales, mins, blocks, bs);
}

#[inline]
fn encode_q5k_superblock(out: &mut [u8], d: f16, dmin: f16, scales: &[u8; 8], mins: &[u8; 8], blocks: &[&AnyBlock], bs: usize) {
